...
# Set up logging
logger = logging.getLogger("menugen.nvidia_description")
...
async def simplify_menu_item_description_nvidia(item: Dict) -> str:
    ...
    logger.info(f"simplify_menu_item_description_nvidia called for item: {item_name}")
    ...
    try:
        ...
        logger.info(f"Calling NVIDIA API for description processing with model: {NVIDIA_TEXT_MODEL}")
        ...
        if not response.choices or not response.choices[0].message or not response.choices[0].message.content:
            logger.warning(f"NVIDIA API returned no description/simplification for: {item_name}. Returning original or empty.")
        ...
        logger.critical(f"Terminating application due to critical error in simplify_menu_item_description_nvidia for item {item_name}.")
